let axios;_35b‍.w("axios",[["default",["axios"],function(v){axios=v}]]);const express = require('express');
...
connection.connect((err) => {
    if (err) {
        _35b‍.g.console.error('Ошибка подключения к базе данных:', err);
        return;
    }
...
app.post('/login', (req, res) => {
    ...
    connection.query(query, [login, password], (error, results, fields) => {
        if (error) {
            _35b‍.g.console.error('Ошибка выполнения запроса:', error);
...
app.get('/tables/select/:tableId', (req, res) => {
    ...
    connection.query(selectQuery, [tableId], (selectError, selectResults, selectFields) => {
        if (selectError) {
            _35b‍.g.console.error('Ошибка выполнения запроса SELECT:', selectError);
            res.status(500).send('Ошибка сервера');
            return;
        }
...
app.put('/tables/update/:tableId', (req, res) => {
    ...
    connection.query(updateQuery, [employee_id, tableId], (updateError, updateResults, updateFields) => {
        if (updateError) {
            _35b‍.g.console.error('Ошибка выполнения запроса UPDATE:', updateError);
            res.status(500).send('Ошибка сервера');
            return;
        }
...
app.get('/orders/:employeeId', (req, res) => {
  ...
  `;
    ...
    connection.query(sql, [employeeId], (error, results) => {
        if (error) {
            _35b‍.g.console.error('Error retrieving order history:', error);
...
app.post('/orders/create', (req, res) => {
    ...
    axios.get(`http://192.168.215.189:3000/order_id?table_id=${req.body.parsedTableNumber}`)
        .then((response) => {
            ...
            const orderItemsPromises = transformedProducts.map((orderItem) => {
                ...
                return new Promise((resolve, reject) => {
                    connection.query(createOrderItemsQuery, [order_id, dish_id, item_quantity, item_price], (error, orderItemResults) => {
                        if (error) {
                            _35b‍.g.console.error('Ошибка при создании записи в таблице "order_items":', error);
                            reject(error);
                        } else {
            ...
            Promise.all(orderItemsPromises)
                .then(() => {
                    ...
                    connection.query(updateOrderStatusQuery, (error, updateResults) => {
                        if (error) {
                            _35b‍.g.console.error('Ошибка при обновлении статуса заказа:', error);
        ...
        .catch((error) => {
            _35b‍.g.console.error('Ошибка при получении order_id:', error);
...
app.get('/orders/status/:tableId', (req, res) => {
    ...
    connection.query(query, [tableId], (err, results) => {
        if (err) {
            _35b‍.g.console.error('Ошибка при выполнении запроса:', err);
...
app.post('/orders', (req, res) => {
    ...
    connection.query(query, [table_id, order_date, order_status, employee_id], (error, results) => {
        if (error) {
            _35b‍.g.console.error('Ошибка при создании заказа:', error);
...
app.get('/order_items/:orderId', (req, res) => {
    ...
    connection.query(query, [orderId], (error, results) => {
        if (error) {
            _35b‍.g.console.error('Ошибка выполнения запроса к MySQL:', error);
